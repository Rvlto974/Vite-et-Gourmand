...
class AvisController {
    ...
    // Changer le statut d'un avis (admin) - AVEC EMAIL
    public function adminUpdateStatus() {
        ...
        if ($avisModel->updateStatus($avis_id, $statut)) {
            // Si l'avis est validé, envoyer un email
            if ($statut == 1 || $statut == 'valide') { // Validé
                // Récupérer les infos complètes de l'avis
                $avis = $avisModel->getById($avis_id);
                ...
                $order = $orderModel->getById($avis['id_commande']);
                ...
                $email = new Email();
                $content = EmailTemplates::validationAvis($user, $menu, $avis);
                
                $email->send(
                    $user['email'],
                    'Votre avis a été validé - Vite & Gourmand',
                    $email->getTemplate($content)
                );
            }
            
            $_SESSION['success'] = 'Statut de l avis mis a jour' . ($statut == 1 ? ' et email envoye' : '');
        } else {
